...
- **NHSN** (supplementary): National Healthcare Safety Network hospital admissions data

## Model Description

This model uses Gradient Boosting Quantile Regression (GBQR) to generate probabilistic forecasts. It combines MCHub target data with NHSN hospital admissions data to provide additional training signal from hospital-based surveillance.
...
- Uses as_of date versioning for NHSN data to prevent data leakage in retrospective runs
...
source src/gbqr_nhsn/.venv/bin/activate

# Basic usage - downloads latest MCHub data from GitHub
PYTHONPATH=$(pwd) python src/gbqr_nhsn/main.py --today_date 2025-12-26

# Use local MCHub data
PYTHONPATH=$(pwd) python src/gbqr_nhsn/main.py --today_date 2025-12-26 --use_local_mchub

# Use versioned MCHub data (as of reference date)
PYTHONPATH=$(pwd) python src/gbqr_nhsn/main.py --today_date 2025-12-26 --use_versioned_mchub

# Short run with reduced bagging
PYTHONPATH=$(pwd) python src/gbqr_nhsn/main.py --today_date 2025-12-26 --short_run
...
```bash
cd /path/to/metrocast-sandbox-2025-2026
PYTHONPATH=$(pwd) src/gbqr_nhsn/.venv/bin/python src/gbqr_nhsn/main.py --today_date 2025-12-26
```
...
Example: `model-output/UMass-gbqr_nhsn/2025-12-27-UMass-gbqr_nhsn.csv`

Quantile levels: 0.025, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.975

## Runtime

- Full run (100 bags): ~50 minutes
- Short run (10 bags): ~5 minutes

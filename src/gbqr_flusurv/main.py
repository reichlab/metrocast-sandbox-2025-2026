"""GBQR model using MCHub + FluSurvNet data.

This model uses:
- MCHub target data (primary source)
- FluSurvNet hospitalization rates (supplementary)
"""
...
def main(today_date: str, short_run: bool, use_local_mchub: bool, use_versioned_mchub: bool):
    """Generate GBQR forecasts using MCHub + FluSurvNet data."""
    ...
    # Model configuration - MCHub + FluSurvNet only
    model_config = ModelConfig(
        model_name="gbqr_flusurv",

        # Only FluSurvNet supplementary source
        use_ilinet=False,
        use_flusurvnet=True,
        use_nhsn=False,

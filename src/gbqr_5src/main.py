"""GBQR model using all 5 data sources (MCHub + ILINet + FluSurvNet + NHSN + NSSP).

This model uses the maximum available training data by combining:
- MCHub target data (primary source)
- ILINet outpatient surveillance
- FluSurvNet hospitalization rates
- NHSN hospital admissions
- NSSP ED visits (additional locations)
"""
...
def main(today_date: str, short_run: bool, use_local_mchub: bool, use_versioned_mchub: bool):
    """Generate GBQR forecasts using all 5 data sources."""
    ...
    # Model configuration - all 5 sources enabled
    model_config = ModelConfig(
        model_name="gbqr_5src",

        # All supplementary sources enabled
        use_ilinet=True,
        use_flusurvnet=True,
        use_nhsn=True,
        use_nssp_extra=True,
    ...
    # Print configuration
    click.echo("Running GBQR model: gbqr_5src")
    click.echo(f"  Reference date: {reference_date}")
    click.echo(f"  Num bags: {model_config.num_bags}")
    click.echo(f"  Data sources: MCHub + ILINet + FluSurvNet + NHSN + NSSP")
    click.echo()

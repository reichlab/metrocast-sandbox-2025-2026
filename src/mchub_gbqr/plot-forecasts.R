...
if (length(args) < 1) {
  ...
  if (length(forecast_files) == 0) {
    stop("No forecast files found and no file specified. Usage: Rscript plot-forecasts.R <forecast_file>")
  }
...
plot_state <- function(state_name, forecasts_df, target_df, historical_df, locations_df, ref_date, model_name = "gbqr", season_year = NULL) {
  ...
  if (nrow(state_locs) == 0) return(NULL)
  ...
  state_forecasts <- forecasts_df |>
    ...
    mutate(location_name = factor(location_name, levels = state_locs$location_name))
  ...
  if (is.null(season_year)) {
    season_year <- if (month(ref_date) >= 8) year(ref_date) else year(ref_date) - 1
  }
  ...
  p <- p +
    ...
    labs(
      ...
      subtitle = sprintf("UMass-%s | Reference: %s | %d location%s",
                        model_name, ref_date, n_locs, if(n_locs > 1) "s" else ""),
      x = "Date", y = "ED visits (%)",
      caption = sprintf("Black: current season | Grey: %d historical season%s | Blue: forecasts (50%% & 95%% intervals)",
...
for (state_name in states) {
  cat(sprintf("  %s\n", state_name))
  p <- plot_state(state_name, forecasts_wide, target_data, historical_data, locations, ref_date, model_name, current_season_year)
  if (!is.null(p)) {
    print(p)
  }
}
...
summary_locs <- c("colorado", "massachusetts", "texas", "minnesota", "georgia", "nyc",
                  "denver", "boston", "houston", "minneapolis", "clt-area")
...
if (length(summary_locs) > 0) {
  ...
  if ("q0.025" %in% names(summary_forecasts)) {
    p_summary <- p_summary + geom_ribbon(
  ...
  if ("q0.5" %in% names(summary_forecasts)) {
    ...
    # Add points for median forecasts
    p_summary <- p_summary + geom_point(
      data = summary_forecasts,
      aes(x = target_end_date, y = q0.5),
      color = "steelblue", size = 2
    )
  ...
  p_summary <- p_summary +
    geom_vline(xintercept = as.numeric(ref_date), linetype = "dashed",
               color = "gray50", alpha = 0.5) +
    facet_wrap(~location_name, scales = "free_y", ncol = 3) +
    labs(
      title = sprintf("UMass-%s Forecasts - Reference Date: %s", model_name, ref_date),
  ...
  summary_file <- file.path(output_dir, sprintf("%s-%s-summary.png", ref_date, model_name))
